...
def test_lda111_pin3_is_its_own_no_connect(tmp_path):
    """Q1 pin 3 is electrically NC but its lead physically exists on the SOP-6.

    It must appear in the netlist as its own no-connect node -- omitting it (as
    the buggy symbol did) or tying it to another net would both be wrong.

    Mounting-hole coverage used to live here too. Removed: MH1-3 are the user's
    call and they are using the padded Mechanical:MountingHole_Pad variant.
    """
    out = tmp_path / 'net.net'
    subprocess.run([KC, 'sch', 'export', 'netlist', '--format', 'kicadsexpr',
                    '-o', str(out), str(conftest.SCH)], check=True,
